<header class="h-header bg-header flex justify-between px-5 items-center">
  ...
  @guest
    <div>
        <a href="{{route("login")}}"><button type="submit" class="btn btn-primary cursor-pointer">{{__("Login")}}</button></a>
        <a href="{{route("register")}}"><button type="submit" class="btn btn-primary cursor-pointer">{{__("Register")}}</button></a>
    </div>
  @endguest
        <x-lang/>
    </div>
